...
import requests,time,random,string,sys,re
...
print("""
        mampir : https://madewgn.my.id/
        contoh link post : "https://www.instagram.com/p/Cg6X457P7FV/" 
        """)
XenziGay = input("masukkan link post > ")
#XenziGay = sys.argv[1];
#time.sleep(2)
bruh = {
    'username': user,
...
berhasil = "Your post is successfully added for free 10 likes. You will get likes in within 1 hour"


response = s.post('https://instagram.qlizz.com/send', headers=headers, data=data).text
#print ("*Your response code:* _"+response+"_")
if berhasil in response:
    print ("*Your response code:* _"+response+"_")
    countdownTimer(60, 00)
else:
    print(response)
    waktu = re.findall(r'\b\d+\b', response)
    w = int(waktu[0])
    countdownTimer(w, 00)
